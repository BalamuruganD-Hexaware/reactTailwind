                ...
                <p>
                  <button
                    ...
                    onClick={() => {
                      if (
                        fname !== '' &&
                        (fname.split('@')[1] === 'hexaware.com' ||
                          fname.split('@')[1] === 'citibank.com' ||
                          fname.split('@')[1] === 'gmail.com')
                      ) {
                        login();
                      }
                    }}
...
if (!localStorage.configurationName) {
  localStorage.configurationName = 'config_classic';
}
...
const MultiAuthProvider = () => {
  ...
  const [configurationName, setConfigurationName] = useState<string>(
    localStorage.configurationName
  );
  const callBack =
  ...
  useEffect(() => {
    ...
    if (
      ...
      fetch(`http://localhost:4000/users/@${fname.split('@')[1]}/`).then(
        result => {
          result.json().then(resp => {
            console.log(resp);
            localStorage.user = fname;
            if (resp.config === 'configurationAuth0') {
              const configurationName = 'config_auth0';
              localStorage.configurationName = configurationName;
